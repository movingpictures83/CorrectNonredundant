...
fq_prob_list = [0.725,
                ...
                0.997786834,
                0.999999999]
fq_char_list = [str(chr(min(int(33 - 10 * math.log10(1 -p)), 73))) for p in fq_prob_list]
...
import PyPluMA
import PyIO
class CorrectNonredundantPlugin:
 def input(self, inputfile):
    self.parameters = PyIO.readParameters(inputfile)
 def run(self):
     pass
 def output(self, outputfile):
  LR_SR_mapping_filename = PyPluMA.prefix()+"/"+self.parameters["LR_SR_mapping_filename"]
  LR_readname_filename = PyPluMA.prefix()+"/"+self.parameters["LR_readname_filename"]
  ...
  #########################################################################################################
    ...
    for SR in SR_ls_ls:
        ...
        if not mismatch == '':
            mismatch_pos_ls = findall(r'\d+',mismatch)
            for ii in range(len(mismatch_pos_ls)):
                mismatch_pos_ls[ii] = int(mismatch_pos_ls[ii])
            #mismatch_pos_ls = map(int, findall(r'\d+',mismatch))
            mismatch_type_ls = findall('>|\+|-', mismatch)
            mismatch_seq_ls = findall('\w+', mismatch)
            j=0
            #print(mismatch)
            #print(mismatch_pos_ls)
            #print(list(mismatch_type_ls))
            #print(list(mismatch_seq_ls))
            for mismatch_type in mismatch_type_ls:
                if mismatch_type == '+':
                    #print("TREVOR")
                    #print(len(list(mismatch_pos_ls)))
                    #print(list(mismatch_pos_ls))
                    #print(mismatch_pos_ls)
                    #print(j)
                    del_pt = list(mismatch_pos_ls)[j] + pos

                    del_pt_set.add(del_pt)

                    temp_del_dt[list(mismatch_pos_ls)[j]] = mismatch_seq_ls[2*j+1]
                    indel_pt_set.add(list(mismatch_pos_ls)[j])
                    ...
                    insert_pt_set.add(list(mismatch_pos_ls)[j])
                    indel_pt_set.add(list(mismatch_pos_ls)[j])
                    ...
                    #print(mismatch_pos_ls)
                    #print(j)
                    p_ls.append(list(mismatch_pos_ls)[j])
                j+=1
